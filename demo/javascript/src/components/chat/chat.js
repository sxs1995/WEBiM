...
module.exports = React.createClass({
  ...
  getInitialState: function() {
    ...
    if (curNode) {
      ...
      $.ajax({
        url:
          WebIM.config.Base_url + "/jntechnician/chatMessages/getStudentName",
        type: "post",
    ...
    Demo.conn.listen({
      ...
      onTextMessage: function(message) {
        ...
        var month = getMonths < 10 ? "0" + getMonths : getMonths;
        var days =
        ...
        var onedate2 =
          ...
          " " +
          onedate.split(" ")[1];
        message.delay = onedate2;
        message.fromName = message.ext.userNickName;
      ...
      onPictureMessage: function(message) {
        ...
        message.url =
          "https://a1.easemob.com/smct-123" + message.url.split("smct-123")[1];
        var mydate = new Date();
        var onedate = mydate.toLocaleString("chinese", { hour12: false });
        var getMonths = mydate.getMonth() + 1;
        var month = getMonths < 10 ? "0" + getMonths : getMonths;
      ...
      onError: function(message) {
        ...
        if (WebIM.config.isWindowSDK) {
          ...
          if (message.type == WebIM.statusCode.WEBIM_CONNCTION_DISCONNECTED) {
            if (
              Demo.conn.autoReconnectNumTotal < Demo.conn.autoReconnectNumMax
            ) {
          ...
          if (message.data && message.data.data) {
            ...
            text =
              WebIM.utils.getObjectKey(WebIM.statusCode, message.type) +
              " " +
              " type=" +
              message.type;
          }
        ...
        if (
          Demo.conn.errorType != WebIM.statusCode.WEBIM_CONNCTION_CLIENT_LOGOUT
        ) {
          if (
            message.type ===
              WebIM.statusCode.WEBIM_CONNECTION_ACCEPT_INVITATION_FROM_GROUP ||
            message.type ===
              WebIM.statusCode.WEBIM_CONNECTION_DECLINE_INVITATION_FROM_GROUP ||
            message.type ===
              WebIM.statusCode.WEBIM_CONNECTION_ACCEPT_JOIN_GROUP ||
            message.type ===
              WebIM.statusCode.WEBIM_CONNECTION_DECLINE_JOIN_GROUP ||
            ...
            if (
              text == "logout" ||
              text == "WEBIM_CONNCTION_SERVER_ERROR  type=8"
            ) {
              text = Demo.lan.logoutSuc;
              Demo.api.NotifySuccess(text);
              // alert('您已下线,请重新登陆!')
              WebIM.config.autoSignIn = false;
              window.location.href = "logoutSuc.html";
              return false;
            } else {
      ...
      onDeliveredMessage: function(message) {
        ...
        for (var targetId in Demo.chatRecord) {
          if (
            Demo.chatRecord[targetId].messages[message.mid] &&
            Demo.chatRecord[targetId].messages[message.mid].status != "Read"
          ) {
            if (msg) {
              if (msg[0]) msg[0].innerHTML = "已送达";
            }
            Demo.chatRecord[targetId].messages[message.mid].status =
              "Delivered";
          }
        }
      },
      ...
      onCreateGroup: function(message) {
        Demo.api.NotifySuccess(
          "Group Created, Group id: " + message.data.groupid
        );
        me.getGroup();
      },
      onMutedMessage: function(message) {
        ...
        if (msg) {
          ...
          var _parentElement = document.getElementById(
              "wrapper" + Demo.selected
            ),
    ...
    Demo.call = new WebIM.WebRTC.Call({
      ...
      listener: {
        ...
        onTermCall: function(reason) {
          ...
          if (
            reason &&
            (reason == "ok" || reason == "success" || reason == "HANGUP")
          ) {
        ...
        onError: function(e) {
          ...
          Demo.api.NotifyError(
            e && e.message ? e.message : "An error occured when calling webrtc"
          );
        }
      }
    });
  ...
  handlePresence: function(msg) {
    ...
    switch (msg.type) {
      ...
      case "createGroupACK":
        Demo.conn.createGroupAsync({
          ...
          success: function(option) {
            ...
            var str = WebIM.utils.sprintf(
              Demo.lan.createGroupSuc,
              option.subject
            );
      ...
      case "leaveGroup": // dismissed by admin
        Demo.api.NotifySuccess(
          `${msg.kicked || "You"} have been dismissed by ${msg.actor ||
            "admin"} .`
        );
        Demo.api.updateGroup();
      ...
      case "unsubscribed": // The other party has removed you from the friend list.
        if ("code" in msg) {
          Demo.api.NotifySuccess(
            WebIM.utils.sprintf(Demo.lan.refuse, msg.from)
          );
        } else {
          // 被删除
          if (msg.from == Demo.selected && !msg.kicked) {
  ...
  getRoster: function(doNotUpdateGroup) {
    var me = this,
      ...
      friends = [];
    $.ajax({
      url: WebIM.config.Base_url + "/jntechnician/chatMessages/getComponyName",
      type: "post",
      contentType: "application/json",
      dataType: "json",
      data: JSON.stringify({
      ...
      success: function(res) {
        Demo.companyName = res.rows;
      }
    });
    //获取最近联系人
    $.ajax({
      url: WebIM.config.Base_url + "/jntechnician/chatMessages/findContacts",
      type: "post",
      contentType: "application/json",
      dataType: "json",
      // async: false,
        ...
        if (res.code == "000000") {
          ...
          for (var o in Demo.strangers) {
            if (Demo.strangers.hasOwnProperty(o)) {
              $.ajax({
                url:
                  WebIM.config.Base_url +
                  "/jntechnician/chatMessages/getStudentName",
                type: "post",
                contentType: "application/json",
                dataType: "json",
                async: false,
                ...
                success: function(res) {
                  strangers.push({ realname: res.rows, username: o });
                }
              });
            }
          }
          var datas = datas2.concat(strangers);
          console.log(datas);
          for (let i = 0; i < datas.length; i++) {
            roster.unshift({
              groups: [],
              jid:
                WebIM.config.appkey + "_" + datas[i].username + "@easemob.com",
          ...
          if (flag) me.setState({ friends: friends });
          else me.setState({ friends: friends, windows: [] });
  ...
  getChatroom: function() {
    ...
    var pageTotal = Math.ceil(
      this.state.chatrooms_totalnum / Demo.api.pagesize
    );
        ...
        success: function(list) {
          ...
          if (list.data) {
            ...
            if (list.data.length > 0) {
              states.chatrooms_totalnum =
                (parseInt(list.params.pagenum[0]) + 1) * Demo.api.pagesize;
  ...
  update: function(cur) {
    ...
    if (Demo.selectedCate == "chatrooms" && node) {
      ...
      if (WebIM.config.isWindowSDK) {
        WebIM.doQuery(
          '{"type":"joinChatroom","id":"' + node + '"}',
          function success(str) {
            Demo.currentChatroom = str;
          },
          function failure(errCode, errMessage) {
            Demo.api.NotifyError(
              "update chatrooms:" + errCode + " " + errMessage
            );
          }
        );
      } else {
        ...
        case "groups":
          ...
          for (var i = 0; i < this.state.groups.length; i++) {
            if (id == this.state.groups[i].roomId) {
              ...
              if (
                this.state.groups[i].roomId == this.state.groups[i].name &&
  ...
  pictureChange: function() {
    ...
    msg.set({
      ...
      onFileUploadComplete: function(data) {
        url =
          (location.protocol != "https:" && WebIM.config.isHttpDNS
            ? Demo.conn.apiUrl + data.uri.substr(data.uri.indexOf("/", 9))
            : data.uri) +
          "/" +
  ...
  audioChange: function() {
    ...
    if (
      (WebIM.utils.getIEVersion === null || WebIM.utils.getIEVersion > 9) &&
      window.Audio
    ) {
  ...
  sendFileImpl: function(type, chatType) {
    ...
    var is_group =
      Demo.selectedCate === "chatrooms" || Demo.selectedCate === "groups"
        ? "groupchat"
        : "singlechat";
    WebIM.doQuery(
      '{"type":"sendFileMessage","to":"' +
        Demo.selected +
        '","message_type":"' +
        type +
        '","group":"' +
