...
var Blacklist = (function() {
  ...
  function _getGroupBlacklistWin(options) {
    ...
    WebIM.doQuery(
      '{"type":"groupSpecification","id":"' + options.roomId + '"}',
      function success(str) {
        if (!str) return;
  ...
  function _addGroupMemberToBlacklistWin(options) {
    ...
    WebIM.doQuery(JSON.stringify(query), sucFn, function failure(
      errCode,
      errMessage
    ) {
  ...
  return {
    ...
    removeGroupMemberFromBlacklist: isWin
      ? _removeGroupMemberWin
      : _removeGroupMember,
    addGroupMemberToBlacklist: isWin
      ? _addGroupMemberToBlacklistWin
      : _addGroupMemberToBlacklist,
    data: function() {
      return data;
    }
  };
...
module.exports = {
  ...
  render: function(node, change) {
    ...
    if (props) {
      ReactDOM.render(
        <Webim config={WebIM.config} close={this.logout} {...props} />,
        this.node
      );
    } else {
      ReactDOM.render(
        <Webim config={WebIM.config} close={this.logout} />,
        this.node
      );
    }
  },
  ...
  releaseChatRecord: function(targetId) {
    ...
    //查询聊天记录
    $.ajax({
      url: WebIM.config.Base_url + "/jntechnician/chatMessages/listMsg",
      type: "post",
      ...
      success: function(res) {
        ...
        for (let obj in datas) {
          let s = [];
          for (let i = 0; i < datas[obj].messages.length; i++) {
            s[datas[obj].messages[i].id] = datas[obj].messages[i];
          }
          datas[obj].messages = s;
        }
        Demo.chatRecord = datas;
      }
    });
        ...
        if (Demo.chatRecord[targetId] && Demo.chatRecord[targetId].messages) {
          ...
          for (var i in Demo.chatRecord[targetId].messages) {
            ...
            if (!Demo.chatRecord[targetId].messages[i].read) {
              Demo.api.appendMsg(
                Demo.chatRecord[targetId].messages[i].message,
      ...
      case "emoji":
        for (var i = 0, l = data.length; i < l; i++) {
          brief +=
            data[i].type === "emoji"
              ? '<img src="' +
                WebIM.utils.parseEmoji(this.encode(data[i].data)) +
                '" />'
  ...
  //     $.ajax({
  //         url: WebIM.config.Base_url + "/jntechnician/chatMessages/getStudentName",
  //         type: "post",
  //         contentType: "application/json",
  //         dataType: "json",
  //         async: false,
  ...
  appendMsg: function(msg, type, status, nid) {
    ...
    this.sentByMe = msg.from === Demo.user;
    // if(msg.from !== Demo.user ){
    //     this.getStudentName(msg.from)
    // }else{
    //     this.getComponyName(Demo.user)
    // }
    var brief = "",
    ...
    var isStranger =
      ...
      !document.getElementById("wrapper" + targetId);
